...
(function () {
  ...
  window.openDispatchModal = async function (opts = {}) {
    ...
    try {
      const { data, error } = await window.supabaseClient
        .from('test_types').select('*').eq('is_active', true)
        .order('required_bsl', { ascending: false }).order('name');
      if (error) throw error;
      _testTypes = data || [];
    } catch (e) {
      if (window.showToast) window.showToast('Lỗi tải loại XN: ' + e.message, 'error');
      return;
    }

    if (!_testTypes.length) {
      if (window.showToast)
        window.showToast('Chưa có loại xét nghiệm nào. Vào Quản lý Phòng Xét nghiệm để thêm danh mục.', 'warning');
      return;
    }
  ...
  function buildModal() {
    ...
    const ttOptions = _testTypes.map((t) =>
    ...
    modalEl.addEventListener('hidden.bs.modal', function () {
      if (S.map) { try { S.map.remove(); } catch (_) {} S.map = null; S.routeLayers = []; }
      wrap.remove();
      setTimeout(() => {
    ...
    document.querySelectorAll('input[name="preset"]').forEach((r) => {
      r.addEventListener('change', () => { S.weights = null; });
    });
  }
  ...
  function rebalanceSliders(changed) {
    ...
    const sumO = o1 + o2;
    if (sumO === 0) { o1 = Math.round(remain / 2); o2 = remain - o1; }
    else { o1 = Math.round((o1 / sumO) * remain); o2 = remain - o1; }
  ...
  window._runDispatch = async function () {
    ...
    S.preset = document.querySelector('input[name="preset"]:checked')?.value || 'balanced';
    ...
    if (isNaN(S.lat) || isNaN(S.lng) || S.lat == null || S.lng == null) {
      if (window.showToast) window.showToast('Chưa có tọa độ điểm sự cố (nhập địa chỉ/vị trí/tọa độ)', 'warning');
      return;
    }
  ...
  function renderResults(result) {
    ...
    if (!ranked.length) {
      ...
      el.innerHTML = `
        ...
        <div class="alert alert-warning">
          <i class='bx bx-error'></i> <strong>Không tìm thấy Phòng xét nghiệm phù hợp</strong> cho "${esc(tt?.name || '')}".<br>
          <small>Nguyên nhân có thể: chưa Phòng xét nghiệm nào đủ cấp an toàn sinh học (cần BSL-${tt?.required_bsl ?? '?'}),
    ...
    const osrmWarn = meta.osrmFailures > 0
    ...
    const cards = ranked.map((lab) => {
      ...
      const srcNote = lab.route.source === 'haversine'
          ...
          <div class="card-body py-2">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
              <div style="min-width:0;">
                ...
                <small class="text-muted d-block">${esc(lab.address || '')}</small>
                <div class="mt-1 d-flex flex-wrap gap-3">
                  ...
                  <small><i class='bx bx-timer'></i> Trả Kết quả: <b>${lab.turnaround_hours != null ? lab.turnaround_hours + 'h' : '—'}</b></small>
                  ${enoughBadge}
                </div>
              </div>
            ...
            <div class="mt-1 d-flex gap-1" style="height:5px;" title="Gần ${lab.scores.gan} · Trống ${lab.scores.trong} · Nhanh ${lab.scores.nhanh}">
  ...
  function labelPreset(meta) {
    ...
    return { urgent: '⚡ Khẩn', balanced: '⚖️ Cân bằng', capacity: '📦 Nhiều mẫu' }[meta.preset] || meta.preset;
  }
  ...
  window._showLabRoute = function (labId) {
    ...
    if (!S.map) {
      ...
      }).addTo(S.map).bindPopup('Điểm sự cố');
    }

    S.routeLayers.forEach((layer) => { try { S.map.removeLayer(layer); } catch (_) {} });
    S.routeLayers = [];
  ...
  window._geocodeAddress = async function () {
    ...
    hint.innerHTML = '<small class="text-muted"><span class="spinner-border spinner-border-sm"></span> Đang tìm địa chỉ...</small>';

    try {
      const url = 'https://nominatim.openstreetmap.org/search?format=json&limit=5&countrycodes=vn&q='
        + encodeURIComponent(q);
      const res = await fetch(url, { headers: { 'Accept-Language': 'vi' } });
      const data = await res.json();

      if (!data || data.length === 0) {
        hint.innerHTML = '<small class="text-danger">Không tìm thấy địa chỉ. Thử nhập cụ thể hơn (thêm quận, thành phố).</small>';
        return;
      }
      ...
      hint.innerHTML = '<small class="text-danger">Lỗi tìm địa chỉ: ' + esc(e.message) + '</small>';
    }
  };
  ...
  window._dispatchUseMyLocation = function () {
    ...
    hint.innerHTML = '<small class="text-muted"><span class="spinner-border spinner-border-sm"></span> Đang lấy vị trí...</small>';
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        hint.innerHTML = '<small class="text-success"><i class="bx bx-check-circle"></i> Đã lấy vị trí hiện tại của bạn.</small>';
      },
      (err) => {
        hint.innerHTML = '<small class="text-danger">Không lấy được vị trí: ' + esc(err.message) + '</small>';
      },
  ...
  console.log('[lab-dispatch-modal.js] ✅ Dispatch Modal sẵn sàng. (window.openDispatchModal)');
})();
